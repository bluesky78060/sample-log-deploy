...
설정 페이지에서 인증 파일을 등록해주세요.`);return}m.classList.add("syncing");const t=new Date().getFullYear(),n=[],s=u.length*(t-b+1);let e=0;B(u);try{for(let i=0;i<u.length;i++){const c=u[i];let g=0,$=[];I(i,"syncing","동기화 중...");for(let r=b;r<=t;r++){try{const l=await window.firestoreDb.getAll(c.type,r),P=`${c.storagePrefix}_${r}`;l&&l.length>0&&(localStorage.setItem(P,JSON.stringify(l)),g+=l.length,$.push({year:r,count:l.length}))}catch(l){(((S=window.logger)==null?void 0:S.error)||console.error)(`${c.name} ${r}년 동기화 오류:`,l)}e++,D(e,s,`${c.name} ${r}년...`)}const C=g>0?`${g}건 완료`:"데이터 없음";I(i,g>0?"success":"",C),n.push({type:c.type,name:c.name,icon:c.icon,totalCount:g,yearsWithData:$,success:!0})}T(n)}catch(i){(((L=window.logger)==null?void 0:L.error)||console.error)("동기화 오류:",i),alert("동기화 중 오류가 발생했습니다: "+i.message)}finally{m.classList.remove("syncing")}}function B(t){let n=`
    ...
    `;t.forEach((s,e)=>{n+=`
        ...
        `}),M.innerHTML=n,d.classList.add("show")}function D(t,n,s){const e=Math.round(t/n*100),o=document.getElementById("syncProgressFill"),a=document.getElementById("syncProgressLabel"),y=document.getElementById("syncProgressPercent");o&&(o.style.width=`${e}%`),a&&(a.textContent=s),y&&(y.textContent=`${e}%`)}function I(t,n,s){const e=document.getElementById(`syncItem${t}`),o=document.getElementById(`syncStatus${t}`);e&&(e.className=`sync-result-item ${n}`),o&&(o.className=`sync-result-count ${n}`,o.textContent=s)}function T(t){let n=0,s="";t.forEach(e=>{n+=e.totalCount;const o=e.totalCount>0?"success":"";e.yearsWithData.length>0&&e.yearsWithData.map(a=>`${a.year}년: ${a.count}건`).join(", "),s+=`

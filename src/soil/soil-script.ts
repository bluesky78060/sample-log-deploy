/**
 * @fileoverview 토양 시료 전용 스크립트 - TypeScript
...
class SoilSampleManager extends BaseSampleManager<SoilSample> {
    // Soil-specific state
    parcels: SoilParcel[];
    ...
    async saveLogs() {
        ...
        this.sampleLogs.forEach(item => {
            if (!item.id) item.id = SampleUtils.generateUUID();
        });
    ...
    // ========================================
    // Firebase 헬퍼 메서드 (Quota 절감용 개별/배치 저장)
    // ========================================

    firebaseSaveRecords(logs: SoilLog | SoilLog[]): void {
        if (!window.firestoreDb?.isEnabled()) return;
        const arr = Array.isArray(logs) ? logs : [logs];
        const year = parseInt(this.selectedYear);
        const promises = arr
            .filter(log => log.id)
            .map(log => window.firestoreDb!.save('soil', year, String(log.id), log as unknown as Record<string, unknown>));
    ...
    firebaseDeleteRecords(ids: string | string[]): void {
        if (!window.firestoreDb?.isEnabled()) return;
    ...
    firebaseBatchSync(): void {
        if (!window.firestoreDb?.isEnabled()) return;
        const snapshot = [...this.sampleLogs];
        if (snapshot.length === 0) return; // 빈 배열은 batchSave가 false를 반환하므로 호출 생략
        window.firestoreDb.batchSave('soil', parseInt(this.selectedYear), snapshot as unknown as Record<string, unknown>[])
            .then((ok: boolean) => {
                if (ok) {
     ...
     */
    protected override _retryCloudSyncAction(): void {
    ...
    async deleteGroup(groupId, baseReceptionNumber?) {
        ...
        if (deleteIds.map(String).includes(String(this.editingLogId))) this.cancelEditMode();
    ...
    addImportedRecord(record: ImportedSoilRecord): SoilLog {
        ...
        // computeNextNumber가 일반 체계(fill=false)로 계산하면서 방금 저장한 성토 레코드를
        // 제외하므로 카운터가 전진하지 않고 성토 행 전부가 '1'로 저장된다 (SAMPL-1-124 적대적 검증).
    ...
    submitForm() {
        ...
        if (this.editingLogId) {
            ...
            this.filterAndRenderLogs();
            // 주소 변경 시 재검증 (addressVerified 초기화 후)
            delete (updatedLog as any).addressVerified;
            this.validateAndMarkLogs([updatedLog]);
            this.cancelEditMode();
    ...
    async validateParcelAddress(lotAddress: string): Promise<boolean | null> {
        ...
        if (SIDO_RE.test(lotAddress)) {
            fullAddress = lotAddress.replace(SHORT_SIDO_RE, (_m, sido, sp) => (SHORT_SIDO_EXPAND[sido] || sido) + sp);
        } else {
            let defaultSido = '';
    ...
    async validateAndMarkLogs(logs: SoilLog[]): Promise<void> {
        const BATCH_SIZE = 5;
        let changed = false;
        ...
        for (let i = 0; i < logs.length; i += BATCH_SIZE) {
            ...
            const results = await Promise.allSettled(
                batch.map(async (log) => {
                    ...
                    const values = verifications
                        .filter((r): r is PromiseFulfilledResult<boolean | null> => r.status === 'fulfilled' && r.value !== null)
                        .map(r => r.value);
            ...
            results.forEach((r, idx) => {
                if (r.status === 'fulfilled' && r.value !== null) {
                    (batch[idx] as any).addressVerified = r.value;
                    // 안전망: saveLogs는 in-place라 고아 참조가 생기지 않지만, 전달된 log가 실제
                    // this.sampleLogs 항목과 다른 참조이면 id로 찾아 동기화(동일 참조면 스킵).
                    const liveLog = this.sampleLogs.find(l => String(l.id) === String(batch[idx].id));
                    if (liveLog && liveLog !== batch[idx]) (liveLog as any).addressVerified = r.value;
                    changed = true;
                }
            });
        }
        ...
        if (changed) {
            ...
            const verifiedLogs = logs.filter(l => (l as any).addressVerified !== undefined);
            if (verifiedLogs.length > 0) this.firebaseSaveRecords(verifiedLogs);
            // 전체 재렌더링 대신 검증 결과 셀만 DOM에서 직접 업데이트
            logs.forEach(log => {
                const invalidClass = (log as any).addressVerified === false;
                this.tableBody?.querySelectorAll(`tr[data-id="${log.id}"] td.col-lot-address`).forEach(td => {
                    ...
                    (td as HTMLElement).title = invalidClass ? '지번 주소가 VWORLD에서 확인되지 않았습니다' : '';
                });
            });
            const invalidCount = logs.filter(l => (l as any).addressVerified === false).length;
            if (invalidCount > 0) {

 ...
 * PathSecurity 인터페이스
 */
interface IPathSecurity {
    checkDangerousPatterns(path: string): PathSafetyResult;
    validateFilename(filename: string): FilenameValidationResult;
    normalizePath(inputPath: string): string;
    checkAllowedExtension(filename: string, allowedExtensions: string[]): boolean;
 ...
 * 경로 보안 검증 유틸리티
 */
const PathSecurity: IPathSecurity = {
    /**
     * 위험한 경로 패턴 검사
     * @param path - 검사할 경로
     * @returns PathSafetyResult
     */
    checkDangerousPatterns(path: string): PathSafetyResult {
...
// ES6 모듈 내보내기
export { PathSecurity };
export type { PathSafetyResult, FilenameValidationResult, IPathSecurity };

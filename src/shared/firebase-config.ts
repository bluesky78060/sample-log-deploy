...
async function initializeFirebase(): Promise<boolean> {
    ...
    try {
        ...
        try {
            ...
            (window.logger?.warn || console.warn)('[Firebase] 오프라인 지원 에러:', error.code, error.message);
        ...
        (window.logger?.error || console.error)('[Firebase] 에러 상세:', (error as Error).message, (error as Error).stack);
        return false;
    }
}
 ...
 */
// @ts-ignore - Firebase compat return type
function getDb(): firebase.firestore.Firestore | null {
    return db;
}

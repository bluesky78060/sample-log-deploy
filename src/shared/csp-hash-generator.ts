...
interface ProjectHashResults {
    [relativePath: string]: ExtractedHashes;
}
 ...
 */
function extractInlineContentHashes(htmlPath: string): ExtractedHashes {
    const html = fs.readFileSync(htmlPath, 'utf8');
    const hashes: ExtractedHashes = {
        scripts: [],
        styles: []
    };
 ...
 */
function generateProjectCSPHashes(projectRoot: string): ProjectHashResults {
    const results: ProjectHashResults = {};
 ...
 */
function buildCSPWithHashes(hashResults: ProjectHashResults): CSPConfig {

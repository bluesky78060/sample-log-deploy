...
interface FirebaseMigrationAPI {
 ...
 */
export async function migrateFirebaseConfig(): Promise<boolean> {
    try {
        // 이미 마이그레이션했는지 확인
 ...
 * @returns {Object|null}
 */
function loadOldConfig(): FirebaseConfig | null {
    try {
...
export async function loadFirebaseConfig(): Promise<FirebaseConfig | null> {
    try {
        ...
        if (secureConfig) {
            return secureConfig as FirebaseConfig;
        }

        // 2. 마이그레이션 시도
        const migrated = await migrateFirebaseConfig();
        if (migrated) {
            const config = await secureStorage.getItem(SECURE_CONFIG_KEY);
            return config as FirebaseConfig | null;
        }
 ...
 */
export async function saveFirebaseConfig(config: FirebaseConfig): Promise<boolean> {
    try {
        // 보안 스토리지에 저장

...
type EventHandler = (this: HTMLElement, event: Event, target: HTMLElement) => void;
...
class EventDelegator {
     ...
     */
    on(eventType: string, selector: string, handler: (event: Event, target: HTMLElement) => void): void {
        if (!this.handlers.has(eventType)) {
            this.handlers.set(eventType, []);

            // 바운드 핸들러 생성 및 저장 (나중에 제거하기 위함)
            const boundHandler: EventListener = (e: Event) => this.handleEvent(eventType, e);
            this.boundHandlers.set(eventType, boundHandler);
            this.container.addEventListener(eventType, boundHandler);
        }

        this.handlers.get(eventType)!.push({ selector, handler: handler as EventHandler });
    }

...
interface NetworkAccessManager {
    STORAGE_KEY: string;
    GATEWAY_STORAGE_KEY: string;
    defaultConfig: NetworkConfig;
    _currentIP: string | null;
    _lastCheck: number | null;
    ...
    getCurrentIP(timeoutMs?: number): Promise<string | null>;
    getSubnetPrefix(ip: string): string;
    checkAccess(): Promise<AccessCheckResult>;
    isAllowed(): Promise<boolean>;
    addAdminIP(ip: string): void;
    removeAdminIP(ip: string): void;
    registerCurrentAsAdmin(): Promise<string | null>;
    resetConfig(): void;
    printStatus(): Promise<{
        ...
        isElectron: boolean;
        allowedGateway: string | null;
    }>;
    showGatewaySetupModal(): Promise<string | null>;
    promptGatewayIfNeeded(): Promise<boolean>;
}

const NetworkAccess: NetworkAccessManager = {
    // localStorage 키
    ...
    getAllowedGateway(): string | null {
        ...
        return (window as any).NETWORK_CONFIG?.ALLOWED_GATEWAY || null;
    },
    ...
    /**
     * 현재 공인 IP 조회
     * @param timeoutMs - 타임아웃 (ms), 미지정 시 설정값 사용
     */
    async getCurrentIP(timeoutMs?: number): Promise<string | null> {
        // 캐시된 IP가 있고 유효하면 반환
     ...
     * 현재 IP를 관리자로 등록
     */
    async registerCurrentAsAdmin(): Promise<string | null> {
    ...
    showGatewaySetupModal(): Promise<string | null> {
        return new Promise((resolve) => {
            ...
            const input = document.getElementById('gatewayIPInput') as HTMLInputElement;
            const saveBtn = document.getElementById('gatewaySaveBtn') as HTMLButtonElement;
            const skipBtn = document.getElementById('gatewaySkipBtn') as HTMLButtonElement;
     ...
     */
    async promptGatewayIfNeeded(): Promise<boolean> {
        // Electron 환경이면 불필요
...
// 전역으로 내보내기
(window as any).NetworkAccess = NetworkAccess;

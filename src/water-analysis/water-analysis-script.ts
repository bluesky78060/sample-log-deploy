 ...
 *
 * 지하수법 시행규칙 [별표 9] 기준:
 * - 일반오염물질 6개 + 특정유해물질 11개 + 생활용수 전용 4개 = 총 21개
...
class WaterAnalysisManager {
    ...
    private async loadTestResults(): Promise<Record<string, WaterTestResult>> {
        ...
        if (window.AnalysisDB?.isReady?.()) {
            try {
                ...
                const map = await window.AnalysisDB.getMap('water', this.selectedYear);
                const typed = (map as Record<string, WaterTestResult>) || {};
                if (Object.keys(typed).length > 0) return typed;
                // IDB 비어 있음 → LS 자가복구 폴백
                (window.logger?.warn || console.warn)('[AnalysisDB] water IDB 빈 맵, LS 자가복구 시도 year=', this.selectedYear);
                const lsData = localStorage.getItem(key);
                if (!lsData) return {};
                const lsParsed = (JSON.parse(lsData) as Record<string, WaterTestResult>) || {};
                if (Object.keys(lsParsed).length > 0) {
                    this._pendingIdbSave = window.AnalysisDB.saveMap('water', this.selectedYear, lsParsed)
    ...
    private saveTestResults(): void {
        const key = `waterTestResults_${this.selectedYear}`;
        let snapshot: Record<string, WaterTestResult>;
        ...
        if (window.AnalysisDB?.isReady?.()) {
            this._pendingIdbSave = window.AnalysisDB.saveMap('water', this.selectedYear, snapshot)
                .catch(e => {
    ...
    private async syncTestResultsFromFirestore(): Promise<void> {
        ...
        try {
            ...
            if (window.AnalysisDB?.isReady?.()) {
                this._pendingIdbSave = window.AnalysisDB.saveMap('water', this.selectedYear, this.testResults)
                    .catch(() => {});
            }
            this.render();
    ...
    private exportToExcel(): void {
        ...
        try {
            ...
            if (livingRows.length > 0) {
                const livingFields = WATER_QUALITY_FIELDS; // 전체 22항목
